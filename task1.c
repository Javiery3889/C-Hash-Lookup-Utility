...
void starttime(); //declaring starttime function
void endtime(); //declaring endtime function
void words(FILE * filename); //declaring word function and passing the argument filename into it
int main(int argc, char const *argv[]){
	...
	else{ // if filename ==0 means user entered entered "small_wordlist.txt" for the second arguement
	words(file); //passing argument file into words function
...
void words(FILE * filename){  
	...
	//A null pointer is returned when EOF is encountered and no more characters can be read
		...
		while( len>0 && isspace(sizeofline[len-1])){ // Searches for "\n" (last character in the password)
			// printf("%d\n",len); // This would print the length of the first password .. Exmaple C\n   (two character)
			len--; 
